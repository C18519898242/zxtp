...
**当前状态：** 阶段 1 已完成。DuckDB 已支持评级统计和逐篇研报；阶段 2 至 4 的盈利预测与价格序列尚未结构化。
...
- `earnings_forecast_consensuses`：结果集 2 的汇总预测数值。
- `earnings_forecast_history`：结果集 3 的年度历史指标。
- `earnings_forecast_institutions`：结果集 4 的日期与机构覆盖记录。
- `earnings_forecast_metadata`：结果集 1 和结果集 5 的年份、状态和发布信息。

**任务：**

- [ ] 在阶段 0 的字段字典确认 `T021` 至 `T038`、`T002`、`T055`、`T059`、`T064`、`T118` 的业务口径和单位。
- [ ] 为每个结果集建立独立测试样本，验证没有将不同粒度的数据合并成一行。
- [ ] 在 `parse_research_ratings` 中按结果集分别写入四张表，并在一次事务中更新。
- [ ] 在 AI Context 增加“盈利预测汇总”小节，显示最新年份的关键共识值与数据日期。
- [ ] 运行完整测试套件。

**验收：** 年度、机构和汇总数据保留各自粒度；空结果集不会删除其他模块的历史数据。
